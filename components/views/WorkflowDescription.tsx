          ...
          <div className="space-y-6">
            {activeSection === "overview" && (
              <Card><CardHeader><CardTitle className="flex items-center gap-2"><Globe className="w-5 h-5" />End-to-End AIO Workflow</CardTitle></CardHeader><CardContent className="space-y-4 text-sm text-muted-foreground leading-relaxed">
                <p>AIO/HSL/MRO Demo System V5.0 is the current production release — a self-contained platform with its own FastAPI backend and PostgreSQL database. V5.0 closes the &ldquo;Live silently truncates on enumeration queries&rdquo; gap that survived V4.6: a new <strong>Exhaustive Live</strong> mode runs Live&apos;s phases 1–3 unchanged and then performs a chunked map-reduce synthesis with strict per-chunk JSON output and in-Python merge by max similarity, guaranteeing every matched AIO reaches the model. Operators get a coverage warning + decremented trust score when any chunk fails. See the <strong>User Guide → Overview</strong> for the full V5.0 changelog and <strong>Search Modes Compendium V5.0</strong> in References for the decision matrix. The pipeline below is unchanged in shape — V5.0 added a new chunked synthesis path next to the existing single-call Live synthesis; Recall, Broad, and Raw work as before. The flow:</p>
