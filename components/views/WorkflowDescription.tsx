          ...
          <div className="space-y-6">
            {activeSection === "overview" && (
              <Card><CardHeader><CardTitle className="flex items-center gap-2"><Globe className="w-5 h-5" />End-to-End AIO Workflow</CardTitle></CardHeader><CardContent className="space-y-4 text-sm text-muted-foreground leading-relaxed">
                <p>AIO/HSL/MRO Demo System V4.6 is the current production release — a self-contained platform with its own FastAPI backend and PostgreSQL database. V4.6 closes the &ldquo;Recall under-resolves on typos / fuzzy queries&rdquo; gap that survived V4.5: typo-tolerant Pass 1d in cue extraction, a Thorough Recall toggle that augments cues with Live-quality LLM-parsed search terms, a backend trigram path with bidirectional substring fallback, and a substrate-chat answer prompt rewritten 1:1 from Live&apos;s prompt. See the <strong>User Guide → Overview</strong> for the full V4.6 changelog. The pipeline below is unchanged in shape — only the Recall (substrate) leg got the new fuzzy + thoroughness layers. The flow:</p>
