          ...
          <div className="space-y-6">
            {activeSection === "overview" && (
              <Card><CardHeader><CardTitle className="flex items-center gap-2"><Globe className="w-5 h-5" />End-to-End AIO Workflow</CardTitle></CardHeader><CardContent className="space-y-4 text-sm text-muted-foreground leading-relaxed">
                <p>AIO/HSL/MRO Demo System V5.0 is the current production release — a self-contained platform with its own FastAPI backend and PostgreSQL database. V5.0 closes the &ldquo;Live silently truncates on enumeration queries&rdquo; gap that survived V4.6 with a new <strong>Exhaustive Live</strong> mode (chunked map-reduce synthesis), and adds an opt-in <strong>Smart Search</strong> auto-mode classifier that picks the optimized search mode for each query. Other V5.0 additions visible in the workflow below: <strong>multi-PDF import</strong> (queue with sequential extraction), <strong>Prompt Library</strong> (24 curated prompts surfaced via Prior Prompts → Library), <strong>token-budget guardrails</strong> on both Live and Recall pipelines, and <strong>concurrent chunk dispatch</strong> for Exhaustive (3× latency win). See the <strong>User Guide → Overview</strong> for the full V5.0 changelog and <strong>Search Modes Compendium V5.0</strong> in References for the decision matrix. The pipeline below is unchanged in shape — V5.0 added a new chunked synthesis path next to the existing single-call Live synthesis; Recall, Broad, and Raw work as before. The flow:</p>
                <ol className="list-decimal list-inside space-y-2">
                  <li><strong>Upload (CSV):</strong> User selects one or more <code className="bg-muted px-1 rounded">.csv</code> files via drag-and-drop or file picker.</li>
                  <li><strong>Upload (PDF, V5.0+):</strong> The PDF import view accepts <em>N PDFs</em> at once. Each file enters a queue card; the backend extracts each PDF to a CSV via Claude Vision sequentially (one Anthropic call at a time to respect rate limits). Per-file actions: download CSV, import to converter, retry, remove. Bulk action: <strong>Import all to converter</strong> appends every successful CSV in one batch. Large PDFs auto-chunk into 100-page slices.</li>
                  ...
                  <li><strong>ChatAIO — five search modes:</strong> Ask natural-language questions about your AIO data using Claude AI. Five modes available: <strong>Recall</strong> (default, memory-augmented), <strong>Live</strong> (bounded fresh retrieval), <strong>Exhaustive Live</strong> (V5.0+ chunked map-reduce, guaranteed completeness), <strong>Broad</strong> (no retrieval — control), <strong>Raw</strong> (raw CSVs — control). Token-budget guardrails on Live + Recall prevent the 200k-token overflow class of error. SSE timeouts (60s first-byte / 30s heartbeat / 180s total) surface failed runs with actionable error messages instead of vague "Load failed".</li>
                  <li><strong>Smart Search (V5.0+ opt-in):</strong> Operator toggles in System Admin → Settings to collapse the multi-button row to a single <strong>Smart Search</strong> button. Each query is classified — enumeration words → Exhaustive Live; freshness words → Live with bypass; comparison → Compare-Modes; single-fact → Live; everything else → Recall. Decision logic mirrors the <code className="bg-muted px-1 rounded">aio-search-auto</code> Claude Code skill, so external agents and in-app users pick modes the same way.</li>
                  <li><strong>Prompt Library (V5.0+):</strong> Admin-managed shared catalog of curated prompts. Ships seeded with 24 atomic exemplar prompts (PM workload 5, bid strategy 4, RFI pipeline 4, subcontractor 4, financial 7). Operators pick from <strong>Prior Prompts → Library</strong> in ChatAIO; admins manage entries via System Admin → Prompt Library tab.</li>
                  <li><strong>Saved Prompts:</strong> Operator-personal bookmarks of frequently used ChatAIO queries. Distinct from the shared Prompt Library — saved prompts are individual; library entries are admin-curated for the team.</li>
                  ...
                  <li><strong>MRO Capture:</strong> Every ChatAIO response is automatically persisted as a Memory Result Object (MRO) — query, cue set, evidence bundle, and answer — and linked back to the matched HSLs as <code className="bg-muted px-1 rounded">[MRO.&lt;uuid&gt;]</code> element slots. Exhaustive runs persist with <code className="bg-muted px-1 rounded">trust_score = 0.85 × coverage</code> so partial-coverage runs carry their uncertainty into future ranking.</li>
                  <li><strong>MRO Reuse + overlap guard:</strong> On subsequent similar queries, Phase 3 reads <code className="bg-muted px-1 rounded">[MRO.*]</code> refs from matched HSL slots and ranks prior MROs by Jaccard × freshness × confidence. The Recall MRO short-circuit (V5.0.1) requires both score ≥ 0.85 AND a Jaccard token overlap ≥ 0.20 between the new query and the cached prior — prevents structurally unrelated priors from spuriously short-circuiting a new query.</li>
                  <li><strong>System Administration:</strong> Manage users, roles, AIO data, HSL data, MRO objects, information elements, saved prompts, prompt library, settings (LLM model + Smart Search), Daily Token Budget, API keys, demo backups (now including <code className="bg-muted px-1 rounded">prompt_library</code> + <code className="bg-muted px-1 rounded">aio_search_quality</code>), and references through the admin panel.</li>
                </ol>
              </CardContent></Card>
            )}
