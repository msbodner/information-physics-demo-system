          ...
          <div className="space-y-6">
            {activeSection === "overview" && (
              <Card><CardHeader><CardTitle className="flex items-center gap-2"><Globe className="w-5 h-5" />End-to-End AIO Workflow</CardTitle></CardHeader><CardContent className="space-y-4 text-sm text-muted-foreground leading-relaxed">
                <p>AIO/HSL/MRO Demo System V5.1 is the current production release — a self-contained platform with its own FastAPI backend and PostgreSQL database. V5.1 ships on top of V5.0&apos;s <strong>Exhaustive Live</strong> mode and adds: a fix for the PDF Import freeze (React useEffect race condition that left the queue stuck on &ldquo;Working…&rdquo;), a comprehensive <strong>Summarize All</strong> that identifies industry/domain, data categories, primary entities, and patterns over the full corpus, persisted PDFs (<code className="bg-muted px-1 rounded">imported_pdfs</code> table with bytea content + System Admin pane), SHA-256 duplicate detection on PDF upload, new <strong>View Imported PDFs</strong> + <strong>Create AIOs from PDFs</strong> buttons, and a new <strong>Bulk HSL Process — V5.1 As-Built</strong> technote in References. The pipeline below is unchanged in shape — V5.1 is a stability + comprehension release; Recall, Live, Exhaustive Live, Broad, and Raw all work as before. The flow:</p>
